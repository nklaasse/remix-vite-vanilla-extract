...
export const unorderedList = style([
  elements.list,
  {
    ...
    selectors: {
      // add spacing for ul elements that are placed immediately after p elements
      [`.${typography.container} .${elements.block} + &`]: {
        marginBlockStart: theme.space[2],
      },

      // add spacing for ul elements that are subitems placed immediately after p elements
      [`.${typography.container} .${listItem} .${elements.block} + &`]: {
        marginBlockStart: theme.space["0.5"],
      },

      // add spacing for ul elements that are placed immediately after heading elements
